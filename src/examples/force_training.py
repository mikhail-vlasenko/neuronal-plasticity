from brian2 import NeuronGroup, Synapses, StateMonitor, SpikeMonitor, prefs
...
Pinv = np.eye(N)*alpha  # ok this is quite non-biological i think
...
def zx(t):
    freqs = np.array([2, 5]) * Hz
    offset = np.array([0, 0])  # 1 for off set is detrimental for some reason
    res = np.zeros_like(t) / second
    for freq, off in zip(freqs, offset):
        res += np.sin(2*np.pi*freq*t + off) / len(freqs)
    return res

# t = np.linspace(0, 1e8, 10) * second
# print(zx(t))
# print(zx1(t))
# exit()
...
def train(t):
    global Pinv
    if t > imin and t < icrit:  # within training interval
        cd = Pinv@neurons.r
        err = neurons.z - zx(t)
        neurons.BPhi -= cd*err
...
fig.tight_layout()
plt.savefig("FORCE_training_example.png")
plt.show()
