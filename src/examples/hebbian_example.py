...
def visualize_connectivity(synapses_E, synapses_I, N_units):
    """
    Visualizes the connectivity of the network.

    Parameters:
        synapses_E (Synapses): The excitatory synapses object.
        synapses_I (Synapses): The inhibitory synapses object.
        N_units (int): Total number of neurons in the network.
    """
    # Create connectivity matrices
    conn_matrix_E = np.zeros((N_units, N_units))
    conn_matrix_I = np.zeros((N_units, N_units))

    # Fill excitatory connectivity matrix
    conn_matrix_E[synapses_E.i[:], synapses_E.j[:]] = 1

    # Fill inhibitory connectivity matrix
    conn_matrix_I[synapses_I.i[:], synapses_I.j[:]] = 1

    # Plot the connectivity matrices
    fig, axes = plt.subplots(1, 2, figsize=(12, 6))

    # Excitatory connectivity
    axes[0].imshow(conn_matrix_E, cmap='Greens', origin='upper')
    axes[0].set_title("Excitatory Connectivity")
    axes[0].set_xlabel("Target Neuron")
    axes[0].set_ylabel("Source Neuron")

    # Inhibitory connectivity
    axes[1].imshow(conn_matrix_I, cmap='Reds', origin='upper')
    axes[1].set_title("Inhibitory Connectivity")
...
visualize_connectivity(synapses_E, synapses_I, N_units)
...
for ws in statemon_synapses_for_assembly_analysis.w.T:
    ...
    for in_assembly, stim_units in zip([in_assembly_A, in_assembly_B],
        ...
        # here we count which units of those that was not stimulated externally.
        # starting with the stimulated ones, we traverse the graph following strong edges
        # then count the number of unique units in the assembly
